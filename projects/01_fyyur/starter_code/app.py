...
class Venue(db.Model):
    ...
    @property
    def serialize_upcoming_shows_count(self):
        return {'id': self.id,
                'name': self.name,
                'city': self.city,
                'state': self.state,
    ...
    @property
    def serialize_shows_details(self):
        return {'id': self.id,
                'name': self.name,
                'city': self.city,
                'state': self.state,
                ...
                'seeking_description': self.seeking_description,
                'website': self.website,
                'upcoming_shows': [show.serialize_artist_venue for show in Show.query.filter(
                    Show.start_time > datetime.now(),
                    Show.venue_id == self.id).all()],
                'past_shows': [show.serialize_artist_venue for show in Show.query.filter(
                    Show.start_time < datetime.now(),
                    Show.venue_id == self.id).all()],
    ...
    def filter_on_city_state(self):
        return {'city': self.city,
                'state': self.state,
                'venues': [v.serialize_upcoming_shows_count
...
class Show(db.Model):
    ...
    @property
    def serialize_artist_venue(self):
        return {'id': self.id,
                'start_time':self.start_time,
                'venue': [v.serialize for v in Venue.query.filter(Venue.id == self.venue_id).all() if v is not None][0],
                'artist': [a.serialize for a in Artist.query.filter(Artist.id == self.artist_id).all() if a is not None][0]
                }
...
def show_venue(venue_id):
    ...
    # data = [v.serialize_upcoming_shows_count for v in venues][0]
    data = venues.serialize_shows_details
...
def artists():
    ...
    data = [artist.serialize_shows_details for artist in artists]
...
def show_artist(artist_id):
    ...
    data = artist.serialize_shows_details
...
def shows():
    ...
    if len(shows) == 0:
      ...
      data = [show.serialize_artist_venue for show in shows] 
    print(data)
